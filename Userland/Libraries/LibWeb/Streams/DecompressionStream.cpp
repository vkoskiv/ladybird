...
ErrorOr<DecompressorContext> decompressor_from_type(Bindings::CompressionFormat format, NonnullOwnPtr<AK::AllocatingMemoryStream> stream) {
    switch (format) {
...
WebIDL::ExceptionOr<JS::NonnullGCPtr<DecompressionStream>> DecompressionStream::construct_impl(JS::Realm& realm, Bindings::CompressionFormat format)
{
    ...
    auto decompression_stream = realm.heap().allocate<DecompressionStream>(realm, realm, move(context));
    ...
    auto compress_algorithm = JS::create_heap_function(realm.heap(), [&realm, decompression_stream](JS::Value chunk) -> JS::NonnullGCPtr<WebIDL::Promise> {
        ...
        auto chunk_view = realm.vm().heap().allocate<WebIDL::ArrayBufferView>(realm, chunk.as_object());
    ...
    auto flush_algorithm = JS::create_heap_function(realm.heap(), [&realm, decompression_stream]() -> JS::NonnullGCPtr<WebIDL::Promise> {
    ...
    auto transform_stream = realm.heap().allocate<TransformStream>(realm, realm);
